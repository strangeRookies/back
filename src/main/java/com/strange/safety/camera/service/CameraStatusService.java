 ...
 * <p>처리 절차:
 * <ol>
 *   <li>camera_login_id로 Camera (또는 CorporateCamera) 조회</li>
...
public class CameraStatusService {
     ...
     * @return 업데이트된 Camera 또는 CorporateCamera 객체 (조회 실패 시 empty)
     */
    public Optional<Object> applyStatusEvent(CameraStatusEventDto dto) {
        ...
        Optional<Camera> cameraOpt = cameraRepository.findFirstByCameraLoginIdAndStatusOrderByIdDesc(cameraLoginId, com.strange.safety.camera.entity.CameraStatus.ACTIVE);
        ...
        // 1-1. Try CorporateCamera 조회
        Optional<com.strange.safety.corporatecamera.entity.CorporateCamera> corpCameraOpt = 
                corporateCameraRepository.findFirstByCameraLoginIdAndStatusOrderByIdDesc(cameraLoginId, com.strange.safety.camera.entity.CameraStatus.ACTIVE);
        if (corpCameraOpt.isPresent()) {
            com.strange.safety.corporatecamera.entity.CorporateCamera corporateCamera = corpCameraOpt.get();
            CameraConnectionStatus previousStatus = corporateCamera.getConnectionStatus();
            CameraConnectionStatus currentStatus = parseConnectionStatus(dto.status());
            Instant detectedAt = dto.resolvedDetectedAt();

            // 2-1. CorporateCamera.connectionStatus 갱신
            corporateCamera.updateConnectionStatus(currentStatus, detectedAt);
            log.info("[CameraStatus] CorporateCamera 상태 갱신: cameraLoginId={}, {} → {}, reason={}",
                    cameraLoginId, previousStatus, currentStatus, dto.reason());

            // 3-1. CAMERA_STATUS_LOGS에 기업용 이력 저장
            CameraStatusLog statusLog = CameraStatusLog.builder()
                    ...
                    .build();
            CameraStatusLog saved = cameraStatusLogRepository.save(statusLog);
            log.info("[CameraStatus] 기업용 상태 이력 저장 완료: logId={}, cameraLoginId={}, status={}",
                    saved.getId(), cameraLoginId, currentStatus);
            return Optional.of(corporateCamera);
        }

        log.warn("[CameraStatus] camera_login_id={}에 해당하는 카메라를 찾을 수 없습니다.", cameraLoginId);
        return Optional.empty();
    }
