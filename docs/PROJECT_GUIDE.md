...
2. **AI 분석:** AI 엔진이 RTSP 프레임을 수신하여 YOLO26n-pose 모델로 작업자의 키포인트를 추출하고, 이를 실행 경로별 `sequence_length` 설정에 맞춘 시퀀스 버퍼로 구성하여 LSTM 모델로 쓰러짐(Faint)을 판단합니다.
...
* **스트리밍 지연 측정 및 성능 비교 가이드:** [LATENCY_MEASUREMENT_GUIDE.md](file:///c:/Users/user/Documents/최종%20쉴더스/strange_back/docs/LATENCY_MEASUREMENT_GUIDE.md)
